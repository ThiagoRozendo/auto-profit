...
export function setupApiDocs(app: INestApplication): void {
  ...
  const document = SwaggerModule.createDocument(app, new DocumentBuilder().setTitle(`${process.env.SERVICE_NAME ?? 'notification-service'} API`).setDescription('AutoProfit distributed service API documentation.').setVersion('1.0.0').build(), { autoTagControllers: true });
...
function renderScalarHtml(specUrl: string): string {
  return `<!doctype html><html><head><title>AutoProfit API</title><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /></head><body><script id="api-reference" data-url="${specUrl}"></script><script src="https://cdn.jsdelivr.net/npm/@scalar/api-reference"></script></body></html>`;
}
