...
export function setupApiDocs(app: INestApplication): void {
  ...
  const document = SwaggerModule.createDocument(app, new DocumentBuilder().setTitle(`${process.env.SERVICE_NAME ?? 'pricing-service'} API`).setDescription('AutoProfit distributed service API documentation.').setVersion('1.0.0').build(), { autoTagControllers: true });
