...
const categories = ["Manutenção", "Documentação", "Peças", "Transporte", "Limpeza", "Outros"] as const;
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {rows.length} despesas • Total <span className="font-semibold text-foreground">{formatBRL(total)}</span>
          </p>
        </div>
        <ExpenseDialog />
      </div>
      ...
      <div className="card-elevated">
        <div className="flex flex-wrap items-center gap-3 border-b border-border p-4">
          <div className="relative min-w-0 flex-1">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar despesas..." className="pl-9" />
          </div>
            ...
            {rows.map((e) => {
              ...
              return (
                <TableRow key={e.id}>
                  ...
                  <TableCell><span className="rounded-md bg-accent/50 px-2 py-0.5 text-xs">{e.category}</span></TableCell>
                  <TableCell>{e.description}</TableCell>
                  <TableCell className="text-right font-semibold tabular-nums">{formatBRL(e.amount)}</TableCell>
                </TableRow>
              );
            })}
            {rows.length === 0 && (
              <TableRow><TableCell colSpan={5} className="py-12 text-center text-sm text-muted-foreground">Nenhuma despesa encontrada.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <form
          className="space-y-4"
          onSubmit={(e) => { e.preventDefault(); toast.success("Despesa cadastrada!"); setOpen(false); }}
        >
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2"><Label>Valor</Label><Input type="number" step="0.01" placeholder="0,00" required /></div>
          </div>
          <div className="space-y-2"><Label>Descrição</Label><Textarea rows={3} placeholder="Descreva a despesa" required /></div>
          <div className="space-y-2"><Label>Data</Label><Input type="date" required /></div>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancelar</Button>
            <Button type="submit" className="gradient-primary text-primary-foreground">Salvar despesa</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
