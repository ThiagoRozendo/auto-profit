...
function NotificationsPage() {
  ...
  const filtered = items.filter((n) => filter === "todas" || (filter === "nao_lidas" && !n.read) || n.type === filter);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{items.filter((i) => !i.read).length} não lidas</p>
        </div>
      ...
      <div className="card-elevated">
        <ul className="divide-y divide-border">
          {filtered.map((n) => {
            ...
            return (
              <li key={n.id} className={cn("flex items-start gap-4 p-5 transition", !n.read && "bg-primary/[0.03]")}>
                <div className={cn("grid h-10 w-10 shrink-0 place-items-center rounded-lg", meta.color)}>
                  <Icon className="h-5 w-5" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-3">
                    <p className="font-medium">{n.title}</p>
                    <span className="shrink-0 text-[11px] uppercase tracking-wider text-muted-foreground">{n.date}</span>
                  </div>
                  <p className="text-sm text-muted-foreground">{n.description}</p>
                </div>
                {!n.read && (
                  <Button size="sm" variant="ghost" onClick={() => setItems(items.map((i) => i.id === n.id ? { ...i, read: true } : i))}>
                    Marcar como lida
                  </Button>
                )}
              </li>
